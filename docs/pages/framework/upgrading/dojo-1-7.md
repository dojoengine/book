...
With Dojo 1.7, Sozo began relying on "proc macros" (procedural macros) to implement specialized Dojo functionality.
...
1. The model does not contain any enum/option, directly in the model `struct` or in any nested data structures. In this case, just use the `DojoStore` trait as explained in the previous chapter about new projects. Already stored data will be preserved as `DojoStore` does the same thing as `Serde` for all data types other than enums.

2. The model contains at least an enum/option (directly in the model `struct` or in nested data structures). In this case, you must keep the old Dojo storage behavior to preserve already stored data. To do that, you must add the `DojoLegacyStore` derive attribute to your model `struct` only.
