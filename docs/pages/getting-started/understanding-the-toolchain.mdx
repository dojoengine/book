---
title: Understanding the Toolchain
description: Learn about the core tools that power Dojo development.
---
...
katana --dev --dev.block-time 2000  # 2 second block time

# Set custom chain ID
katana --dev --chain-id 0x123456

# Persist chain state to disk
katana --db-dir ./katana-db

# Start with more accounts
katana --dev --accounts 20

# Disable auto mining (manual mining mode)
katana --dev --no-mining

# Enable mining on demand
katana --dev --mine-on-demand

# Set custom gas limits
katana --dev --dev.gas-limit 100000000
```
...
torii --http.port 8081

# Set custom port for gRPC server
torii --grpc.port 8082

# Start from a specific block
torii --start-block 100

# Index historical data from genesis
torii --index-from-genesis

# Set maximum number of events to process per block
torii --events-chunk-size 1024

# Configure database URL (for external databases)
torii --database-url "postgres://user:pass@localhost/db"

# Set log level
torii --log-level debug

# Enable prometheus metrics
torii --metrics

# Set custom metrics port
torii --metrics.port 9090
```
...
See [configuration guide](/framework/configuration) for more information about Sozo profiles.
...
2. **Katana** provides a local Starknet sequencer to deploy and test on
