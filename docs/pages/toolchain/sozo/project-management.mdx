...
`clean` is used to remove the build artifacts to ensure you're not keeping old files that are not overwritten by the [build](#sozo-build) command.
...
3. Deploying your World using [profile options](#profile-configuration), where configuration like `rpc-url` are set in the profile.
...
The `sozo account` command allows managing account contracts for development and deployment.
It provides functionalities to create new account configurations, deploy them, and fetch account configuration for already existing accounts.
