...
Examples: (a) `set_block_number(10)`,
...
Examples: (a) `set_caller_address(ContractAddress::from([1, 2, 3]));`
...
Examples: (a) `set_contract_address(ContractAddress::from([4, 5, 6]));`
...
Examples: (a) `set_sequencer_address(ContractAddress::from([7, 8, 9]));`
...
Examples: (a) `set_block_timestamp(100);`
...
Examples: (a) `set_account_contract_address(ContractAddress::from([10, 11, 12]));`
...
Examples: (a) `set_max_fee(1000000000000000000) sets the maximum fee to 1 ETH.
...
Examples: (a) `set_transaction_hash(123456789);`
...
Examples: (a) `set_chain_id(0x01) sets the chain ID to 1.`
...
Examples: (a) `set_signature([1, 2, 3]);`
...
Examples: (a) `set_block_hash(123, 0x...BlockHash...) sets the block hash for block number 123.`
...
Examples: (a) `let (keys, data) = pop_log_raw(0x...ContractAddress...)?; pops the earliest logged event for the contract.`
...
Examples: (a)let event = pop_log<MyEvent, starknet::Event<MyEvent>>(0x...ContractAddress...)?; pops the earliest logged event for the contract as a MyEvent type.
...
Examples: (a) pop_l2_to_l1_message(address: ContractAddress) -> Option<(felt252, Span<felt252>)>:
