    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {posts.map((post: BlogPost, index) => (
                <div
                    key={index}
                    className="group flex flex-col rounded-xl bg-[#0c0c0c] bg-opacity-30 backdrop-blur-lg overflow-hidden border border-white/5 hover:border-red-500/20 transition-all duration-300"
                >
                    <a href={post.path} className="flex flex-col h-full">
