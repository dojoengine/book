...
:::warning
Windows is not natively supported. We recommend [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) for Windows development.
:::
...
- **glibc**: Version 2.33 or higher (required for Katana)

:::note
Katana is not available on Ubuntu 20.04 LTS, Debian 10 Buster, CentOS 7, or below due to glibc requirements.
:::
...
See the [Dojo configuration](/framework/configuration#project-manifest-scarbtoml) documentation for more info.
:::

### Installing with `dojoup`

[`dojoup`](https://install.dojoengine.org) is Dojo's dedicated package manager that automatically synchronizes compatible versions.
...
If you prefer to build from source, you can compile the tools directly from the repository:
...
# Build Torii
cargo install --path ./torii/bin/torii --locked --force

# Build Sozo
cargo install --path ./sozo/bin/sozo --locked --force
```

:::note
Building from source will install the binaries at `~/.cargo/bin`
:::

### Installing Individual Components

Each component of the Dojo toolchain can also be installed separately:
...
For comprehensive details on using Claude Code skills with Dojo, see our dedicated [AI installation guide](/installation).
...
With Dojo installed, you're ready to explore what you can build with provable games.
See the [Dojo overview](/overview) to understand the framework's capabilities and architecture.
