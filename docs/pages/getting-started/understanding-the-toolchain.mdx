...
For detailed configuration options, see the [Katana configuration reference](/toolchain/katana/configuration).
...
Learn more about Torii's gRPC interface in the [Torii gRPC guide](/toolchain/torii/grpc).
...
For complete configuration options, see the [Torii configuration reference](/toolchain/torii/configuration).
...
For complete details on all Sozo commands, see the [Sozo reference documentation](/toolchain/sozo).
...
### Sozo Profiles and Configuration

Sozo uses profiles to manage different deployment environments, allowing you to maintain separate configurations for development, staging, and production deployments.

#### Profile Structure

Each profile is defined in a `dojo_<PROFILE>.toml` file in your project root:
...
Each profile configuration file defines environment-specific settings:

```toml
[world]
name = "My Game"
description = "An awesome Dojo game"
seed = "my-game-123"  # Changes the World address

[namespace]
default = "mygame"  # Default namespace for the world
...
[writers]
"mygame" = ["mygame-actions"]  # namespace = [list of contracts with write access]
```

#### Using Profiles

Different deployment commands use different profiles:

```bash
# Development (uses dojo_dev.toml)
sozo build
sozo migrate
sozo execute my-action spawn

# Staging (uses dojo_staging.toml)
sozo build --profile staging
sozo migrate --profile staging
sozo execute my-action spawn --profile staging

# Production (uses dojo_mainnet.toml)
sozo migrate --profile mainnet
```

#### Profile Management Best Practices

1. **Required Profile**: You must have a `dojo_dev.toml` file as your default development profile.

2. **Security**: Never commit private keys to version control. Consider using environment variables:
...
3. **World Seeds**: Use different seeds for each environment to avoid address conflicts:
...
4. **Namespace Organization**: Use consistent namespacing across environments:
...
[writers]
"mygame" = ["mygame-actions", "mygame-systems"]
"admin" = ["mygame-admin"]
```

For complete details on profile configuration options, see the [Dojo configuration reference](/framework/configuration).
...
For more information about Saya, see the [Saya documentation](/toolchain/saya).
...
Configuration sources are prioritized in this order (highest to lowest):
...
For detailed configuration information for each tool, see:

- [Dojo configuration](/framework/configuration)
- [Katana configuration](/toolchain/katana/configuration)
- [Torii configuration](/toolchain/torii/configuration)
...
- [Katana documentation](/toolchain/katana)
- [Torii documentation](/toolchain/torii)
- [Sozo documentation](/toolchain/sozo)
- [Saya documentation](/toolchain/saya)
