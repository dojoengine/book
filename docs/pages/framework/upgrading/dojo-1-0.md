...
For human readability, namespaced resources can also be identified by what's called a resource tag, which is a combination of the namespace and the resource name: `namespace-resource_name`.
The resource tag can be used to obtain the dojo selector of the resource.
...
See the [world permissions documentation](./world/permissions) for detailed information about permission management.
...
On the torii side, you can start it from the CLI or using a configuration file with the `historical_events` options, by providing resource tags of events you want to keep historical.
...
You can find detailed information about the configuration file in the [configuration documentation](./configuration).
