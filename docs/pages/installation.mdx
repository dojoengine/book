...
Katana can be installed via the same methods as other Dojo tools:
...
[Slot](https://github.com/cartridge-gg/slot) is an alternative deployment tool for Dojo worlds that simplifies the deployment process.
While `sozo` manages development and testing, Slot provides streamlined production deployments.

### Installing Slot

Slot is distributed as a pre-built binary:

```bash
# Download and install the latest version
curl -L https://slot.cartridge.gg/install | bash
```

### Using Slot

For detailed usage instructions and deployment workflows, see the [deployment tutorials](/tutorials/deploy-using-slot/main).
