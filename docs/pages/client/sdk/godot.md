...
However, the extension supports it and uses wrapper classes to store and show the data.
...
Copy the files you downloaded into an ‘addons’ folder.
If it doesn’t exist, create one as a child of the root folder.

#### Setup ToriiClient
- Add `ToriiClient` node to your scene tree.
- Connect the client
  ```gdscript
...
- Register a subscription
  :::tip
  If `ToriiClient.subscribe_entity_updates` is used with a empty `DojoClause`, it retrieves ALL entity updates.
  :::
    ```gdscript
    ...
    func _register_sub():
        ...
        _dojo_callback.on_error = func(_err): printt("DojoError", _err) # Can be a lambda too
        var dojo_worlds:Array = ["0x..."]
        if torii_client.is_valid(): # if used in the same method as `torii_client.connect`, you can use the `_success` value instead.
...
Add `DojoSessionAccount` node to your scene tree.
...
func trigger_login():
	...
	# Generate private key. If a previously generated key is used, you can create a session without logging in again.
	_priv_key = ControllerHelper.generate_private_key() # Make sure to save it or keep available for next step
...
```
Once session is successfully registered, we fetch the session
...
```gdscript
	...
	dojo_session_account.create_from_subscribe(
		...
		"https://api.cartridge.gg" # Optional, it defaults to `https://api.cartridge.gg`
	)
```
...
Subscriptions can be created through `ToriiClient`, when a subscription is sucessfully created it will retrieve a `sub_id`.

This `sub_id` is required to update/cancel the subscription.

Subscriptions are automatically cancelled when closing the game.

:::note

Refer to in-editor documentation to see the rest of the subscriptions.
...
Every first parameter is a `Callable`.
This can be a lambda function, constructed using Callable.new() to use a function of another object, or any function in the script.
...
func setup_subscriptions() -> void:
	...
	_dojo_callback_message.on_update = callback.bind("Message") # In this case, we add a extra parameter to the callback
...
:::tip
If empty query is used, it will retrieve ALL entities on ALL worlds that Torii is indexing.
:::
:::note
`DojoQuery` if for retrieving entities only, for the rest of queries check in-editor documentation of `ToriiClient`
:::
```gdscript
    ...
    for model in entity["items"]:
        for key in model:
            ...
            if data.has("dojo_starter-Position"):
                ...
                # This will differ according to your contract models/abi.
                var position = Vector2(position_model['vec']['x'], position_model['vec']['y'] )
                var id : String = position_model['player']
                printt(id, position)
```
All queries follows the Builder Pattern, which allows for a flexible and readable way to construct complex queries.
```gdscript
	var query : DojoQuery = DojoQuery.new()
	...
	# Alternatively you can create the query in one line thanks to the Builder Patter used
	
	var query:DojoQuery = DojoQuery.new().with_clause(MemberClause.new().member("id").op(MemberClause.ComparisonOperator.Eq).hex("0x...").model("dojo_starter-Player") ).models(["dojo_starter-Moves", "dojo_starter-Position"]) 
	var data:Dictionary = torii_client.entities(query)
	var _player_items:Array = data['items']
	printt("ITEMS", _player_items)
...
In cases like the Dojo Starter Contract we used its custom struct `Vec2` to convert to Godot's `Vector2`.
...
`MacOS` builds are not universal so it should work on Intel and Apple Silicon, but will require different builds.
An universal build could be done, but we weren't able to properly test them.
:::
...
It uses a custom contract based on `dojo-starter` but with some testing/experimental things to test the extension.
:::
