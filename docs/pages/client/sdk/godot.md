...
Built on the foundational [dojo.c SDK](../c), Dojo.godot provides native Godot integration for blockchain functionality.
...
- **Wallet Authentication**: Secure connection to Cartridge Controller burner accounts.
...
Used for system calls to smart contracts.
...
Fetch the current world state using queries:
...
- **Real-time Updates**: Synchronization between world state and game visuals.
...
The demo connects to a testnet deployment, demonstrating real blockchain integration in a simple 2D movement game.
