...
Cairo testing cheat codes allow you to set and manipulate various execution context variables, such as block number, caller address, and contract address, to test your contracts in different scenarios.

This guide explores each cheat code in detail, providing explanations and examples to help you understand how to use them effectively in your tests.
For more information on testing with Dojo, see the [main testing guide](./index.md).

## `set_block_number`

This cheat code sets the current block number to the specified value, allowing you to simulate different block heights for testing purposes.

This is helpful to test a contract's behavior at a specific block height, such as checking if a certain function is only callable after a certain block number.
...
This cheat code sets the caller address to the provided contract address, enabling you to test contract interactions with different callers.
...
This cheat code sets the contract address to the provided value, allowing you to test contract deployment and interactions.

It is important to note that any test function is considered a contract, which by default uses the `0` address.
Using `set_contract_address` allows you to mock the current address of the testing function, making it useful to call other contracts that may use `get_caller_address`.
...
This cheat code sets the block timestamp to the specified value, allowing you to test contract behavior at different points in time.
...
This cheat code sets the transaction version to the provided value.
...
This cheat code sets the account contract address to the provided value, allowing you to test contract interactions with different account contracts.
You can apply this when simulating a scenario where a contract is called by a different account contract.
...
This cheat code sets the maximum fee to the provided value, enabling you to test contract behavior with different fee structures.
...
This cheat code sets the transaction hash to the provided value, allowing you to test contract behavior with different transaction hashes.
You can apply this when:
...
This cheat code sets the chain ID to the provided value, enabling you to test contract behavior on different chains.
You can apply this when:
...
This cheat code sets the nonce to the provided value, allowing you to test contract behavior with different nonces.
You can apply this when:
...
This cheat code sets the signature to the provided value, enabling you to test contract behavior with different signatures.
...
This cheat code pops the earliest unpopped logged event for the contract, returning the event data and keys.
...
This cheat code pops the earliest unpopped logged event for the contract as the requested type, deserializing the event data into the specified type.
You can apply this when:
...
This cheat code pops the earliest unpopped L2 to L1 message for the contract, returning the message data and keys.
You can apply this when:
