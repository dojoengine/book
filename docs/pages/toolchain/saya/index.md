...
Once the block execution is proven, Saya can settle the block either by verifying it on a settlement layer and updating the state of the core contract, or by posting the data to a data availability layer.
...
:::note
Currently, Saya is not generalized, which means that each running Katana sequencer needs to have its own Saya instance.
This will change in the future, and Saya will be able to aggregate multiple Katana sequencers blocks into a single instance.
:::
