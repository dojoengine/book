...
- `model` (namespaced): A Component defines data that can be stored in the world.
- `event` (namespaced): An event also defines data, but meant to be stored offchain.
- `contract` (namespaced): Where you define your business logic, and interact with the world to write/read components and emit events. A function into a contract is called a `System`, which is an entrypoint for users to interact with the world.
...
To read/write data to components, you have to import the `ModelStorage` trait:
...
The important concept to keep in mind is that the data stored in the world are identified by the `keys` you are adding using `#[key]` in components and events.
A component/event can have one or multiple keys.
When those keys are hashed, it's called the `entity ID`.
...
However, they are subjected to the same namespace rules as components.
...
As mentioned previously, all the resources are permissioned.
For more details on permissions, see the [world permissions guide](./framework/world/permissions).

Some examples of the permission API:
...
And by default, events behave like components, which means only the latest state is kept.
...
As we've seen, resources like contracts, components and events are namespaced, so you have to specify the namespace you want to use when testing.

For complete testing documentation, see the [testing guide](./framework/testing).
...
2. Then, `Sozo` will register all the resources. Registering the resources means that all components/events/contracts will be declared and deployed onchain. None of those contracts are using constructor calldata, hence `Sozo` can deploy them without prior inputs. All resources are registered to the world and deployed through the world contract.
3. Once all the resources are registered, `Sozo` will synchronize the permissions that are given in the configuration file.
4. Finally, `Sozo` will initialize all the contracts. Since the contracts initialization function is very likely to interact with components, at this point all permissions are synchronized and the world is ready to use.
...
Once again, the same component or event type can be registered multiple times into different namespaces, which will yield different resources.
...
You can find detailed information about the configuration file in the [configuration guide](./framework/configuration).
...
# Grant/revoke a owner/writer permission to any resource:
...
To ensure the nodes serving mainnet data are accepting `Sozo` requests, you must set the `world_block` key in the configuration file.
