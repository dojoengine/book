...
See [configuration documentation](./framework/configuration) for more information about Sozo profiles.
...
# Or start Katana on a different port
katana --dev --rpc-port 5051
```

#### Configuration Issues

```bash
# Verify your katana.toml is valid
katana --config katana.toml --dry-run
...
```bash
# Get the actual World address from your migration
sozo inspect

# Update torii_dev.toml with the correct world address
# Then restart Torii
...
```bash
# Clear Torii's database and restart
rm -rf torii.db/
...
# If Katana is on a different port or host
torii --rpc http://localhost:5051 --world <WORLD_ADDRESS>
```
...
ls -la | grep -E "(Scarb.toml|dojo_dev.toml)"

# If missing, initialize a new project
sozo init my-project
```
...
# If account issues persist, use a fresh account from Katana
# Update dojo_dev.toml with new account details
```
...
```bash
# Grant necessary permissions after migration
sozo auth grant di-actions di <WORLD_ADDRESS>
...
```bash
# If deploying to the same address fails
# Either use a different seed in dojo_dev.toml
seed = "new_unique_seed"

# Or clean deploy to a fresh world
sozo migrate --dev --name new_world
```
...
curl http://localhost:5050     # Katana health check
curl http://localhost:8080     # Torii health check

# Check process status
ps aux | grep -E "(katana|torii)"
```

#### Review Logs

```bash
# Run tools with verbose logging
katana --dev -vvv
torii --world <WORLD_ADDRESS> --log-level debug
sozo migrate --dev -v
```
...
# Start fresh
katana --dev
# Wait for Katana to fully start, then:
sozo migrate --dev
torii --world <WORLD_ADDRESS>
```
...
Now that you understand the toolchain, you're ready to learn about the [development workflow](./development-workflow) and start building more complex applications.
