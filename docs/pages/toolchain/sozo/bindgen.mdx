...
Control where bindings are generated:

```bash
sozo bindgen --binding-target typescript --output-dir ./src/contracts/generated
sozo bindgen --binding-target unity --output-dir ./Assets/Scripts/Dojo
```

**Available binding targets:**
- `typescript` - Modern TypeScript definitions
- `typescript-v2` - Alternative TypeScript format
- `unity` - C# classes for Unity
...
```
bindings/
└── {world_name}.ts     # For TypeScript targets
└── *.cs                # For Unity targets
└── *.h *.cpp           # For Unreal targets
```
