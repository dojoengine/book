...
**Note**: You can directly run `sozo migrate` without the need of running `sozo migrate plan`. You should run `sozo migrate plan` if you want to generate the manifests file only and do a dry run without sending transactions to the chain.
