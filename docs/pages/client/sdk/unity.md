...
2. Import model bindings: Locate the `bindings/unity/Models` folder within your Dojo project, and drag the desired `model` files from this folder into your Unity project.
    The `Synchronization Master` will automatically detect and load these models for seamless data exchange.
...
Once we have an account, we must execute a call to a Dojo system.
To do this, we must first teach our Unity project about our Dojo contracts.
...
Let's consider a contract named `PlayerActions` within our Dojo project:
...
-   `new dojo.Call{ ... }`: Creates a new call, where the `selector` is the name of the system function, and `calldata` are the parameters serialized as FieldElements.
-   `account.ExecuteRaw(new dojo.Call[] { ... })`: Executes the call, where `account` can be either a simple account or a burner account.
