...
## Advanced ECS Design Patterns

Understanding how to model your game state using the Entity Component System (ECS) architecture is crucial for building efficient Dojo games.
Different approaches to ECS modeling can significantly impact your game's performance and complexity.

### Chess Game ECS Approach

When building a chess game, there are several ways to approach ECS modeling.
One interesting approach is to treat the game board differently than you might initially expect:

> Every square of the chess board will be treated as an entity.
> If a piece exists on a square position, that position will hold that piece.

This approach offers several advantages:

1. **Simplified Collision Detection**: Each square is an entity, making it easy to check if a square is occupied
2. **Natural Board State**: The board state is inherently represented by the entity positions
3. **Move Validation**: Piece movement can be validated by checking target square entities
...
This modeling approach demonstrates how ECS can be used creatively to represent complex game states in ways that might not be immediately obvious but offer significant implementation benefits.

For a complete implementation of this chess example, including game logic and move validation, see the [onchain chess tutorial](/tutorials/onchain-chess/0-setup).

## Systems

A dojo system is a Starknet contract annotated with the `#[dojo::contract]` attribute.
...
## Contract Compilation and Environment Setup
...
Enough theory! Let's build the Dojo project!
...
Success! [Katana](/toolchain/katana) should now be running locally on your machine.
