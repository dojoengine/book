...
void on_entity_update(FieldElement key, CArrayStruct models) {
    printf("Entity updated: 0x");
    for (int i = 0; i < 32; i++) {
...
// Subscribe to all entity updates (no filter)
COptionClause no_filter = { .tag = NoneClause };
ResultSubscription sub = client_on_entity_state_update(
    client,
    no_filter,
    &on_entity_update
);
...
FieldElement contract_addr;
hex_to_bytes("0x01385f25d20a724edc9c7b3bd9636c59af64cbaf9fcd12f33b3af96b2452f295", &contract_addr);

Call move_call = {
    .to = contract_addr,        // Address as FieldElement
    .selector = "move",         // Function name
    .calldata = {
        .data = calldata_array,
        .data_len = 2
    }
...
// Execute transaction
ResultFieldElement result = controller_execute_raw(account, &move_call, 1);
if (result.tag == OkFieldElement) {
    printf("Transaction hash: 0x");
    for (int i = 0; i < 32; i++) {
        printf("%02x", result.ok.data[i]);
    }
    printf("\n");
}
```
...
All dojo.c functions return `Result` types that must be checked:

```c
ResultToriiClient res = client_new(torii_url, world);
switch (res.tag) {
    case OkToriiClient:
        // Success - use res.ok
        client = res.ok;
        break;
    case ErrToriiClient:
        // Error - check res.err.message
        printf("Error: %s\n", res.err.message);
        return 1;
}

// Controller account example
ResultControllerAccount controller_res = controller_account(policies, policies_len, chain_id);
if (controller_res.tag == OkControllerAccount) {
    ControllerAccount *account = controller_res.ok;
} else {
    printf("No existing session: %s\n", controller_res.err.message);
}
```
...
Query query = {
    .pagination = {
        .cursor = { .tag = Nonec_char },
        .limit = { .tag = Someu32, .some = 100 },
        .direction = Forward,
        .order_by = { .data = NULL, .data_len = 0 }
    },
    .clause = {
        .tag = SomeClause,
        .some = {
            .tag = Keys,
            .keys = {
                .keys = { .data = key_array, .data_len = 1 },
                .pattern_matching = FixedLen,
                .models = { .data = NULL, .data_len = 0 }
            }
        }
    },
...
ResultPageEntity entities = client_entities(client, query);
```

### Burner Account Management

```c
// Create burner account funded by master account
FieldElement burner_key = signing_key_new();
ResultAccount burner = account_deploy_burner(provider, master_account, burner_key);

if (burner.tag == OkAccount) {
    printf("Burner deployed: 0x");
    FieldElement addr = account_address(burner.ok);
    for (int i = 0; i < 32; i++) {
...
dojo.c manages memory automatically for most operations, but some resources need explicit cleanup:

```c
// Free clients when done
client_free(client);

// Free accounts when done
account_free(account);

// Result types don't need manual cleanup
```
...
```c
// Set log level (if supported by your build)
set_log_level("debug");

// Check function return values
if (result.tag == ErrToriiClient) {
