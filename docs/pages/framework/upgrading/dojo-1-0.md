...
First, when you are inside a dojo contract (define with `#[dojo::contract]`), you have to retrieve the World contract's instance.
...
This `world` instance provides you some functionalities to interact with the World contract and its data.
...
Events are only emitted by the World contract, and never stored onchain.
...
1. First of all, `Sozo` will migrate the World contract itself.
...
The model introspection functionality is provided by the [model introspection system](/framework/models/introspection).
...
- World contract's metadata are not uploaded yet, this should be added back soon.
