...
To start, let us create a new project to run locally on your machine.
...
Congratulations!
You now have a local Dojo project!
...
It is the ideal starting point for a new project and equips you with everything you need to begin hacking.

### Anatomy of a Dojo Project

Inspect the contents of the `dojo-starter` project, and you will notice the following structure (excluding the non-Cairo files):
...
Next, let us have a look at the `src/models/position.cairo` file.
...
Let us examine the `src/systems/actions.cairo` file.
...
pub mod actions {
    ...
    impl InternalImpl of InternalTrait {
        ...
        // This function is handy since the ByteArray cannot be const.
...
Now, let us deploy it to [Katana](/toolchain/katana)!
...
Let us discuss the `Scarb.toml` file in the project.
...
With your local world address established, let us delve into indexing.
...
When you run the command, you will see terminal output that looks something like this:
...
Next, let us use the GraphQL IDE to retrieve data from all the models that have been registered.
...
Awesome, now let us work with subscriptions to get real-time updates.
Let us clean up your workspace on the GraphQL IDE and input the following subscription:
...
By running this command, you have executed the spawn system, resulting in the creation of a new entity.
